...
macro( add_service projectname )
	if( NOT USE_QT5 )
		...
		foreach(qt_lib ${qt_libs})
			...
			include_directories(${${qt_lib}_INCLUDE_DIR} ${${qt_lib}_INCLUDE_DIRS})
		endforeach()
	endif()
...
#	message(STATUS "projectname: ${projectname}")
	...
	if( hwtest )
		install( TARGETS ${projectname} RUNTIME DESTINATION ${CLIENT_DATA_DIR}/hwtests )
	else()
...
##			message(STATUS "${bycicle_path}" )
##			message(STATUS "${bycicle_path_final}" )
...
#	set_target_properties(${projectname} PROPERTIES AUTOMOC TRUE)
...
#		add_test( NAME ${projectname} COMMAND ${CMAKE_COMMAND} -E environment )
		if( WIN32 )
			...
			foreach(b ${bycicle_path})
				string(REPLACE "\\" "/" b ${b})
				string(REGEX REPLACE "^(.+)/lib$" "\\1/bin" b ${b} )
				list(APPEND bycicle_path_rel ${b})
			endforeach()
...
#	list(APPEND add_lib_internal_deps ${ARGN})
#	message(STATUS "ARGN: ${ARGN}")
#	message(STATUS "add_lib_internal_deps: ${add_lib_internal_deps}")
...
#	message(STATUS "build_project_tree_Qt5_LRELEASE_EXECUTABLE ${Qt5_LRELEASE_EXECUTABLE}")
...
#			add_custom_target(${projectname}_protobuf_autogen_${proto_item_NAME} DEPENDS ${protos_out})
		endforeach()
			...
			foreach(ts ${tssr})
					...
					if(${check_ts} GREATER 0)
						...
						add_custom_command(
								...
								ARGS "${ts}" -qm "${qm}")
#						add_custom_target(${projectname}_tr_${lang} DEPENDS ${qm})
					endif()
			endforeach()
		endforeach()

		add_custom_target(${projectname}_tr ALL DEPENDS ${qms})
...
macro( init_libs )
	...
	foreach( dep ${ARGN} )
		...
		if( ${dep} MATCHES ${PROJECT_NAME_GLOBAL} )
			...
			include_directories( ${${dep}_INCLUDE_DIR} ${${dep}_INCLUDE_DIRS} ${${depUp}_INCLUDE_DIR} ${${depUp}_INCLUDE_DIRS} )
			list( APPEND libs ${${dep}_LIBRARIES} ${${dep}_LIBRARY} ${${depUp}_LIBRARIES} ${${depUp}_LIBRARY} )
...
function( extract_project_options _sources _dependencies )
	set( srcs )
	set( deps )
#	message(STATUS "extract_project_options->ARGN: ${ARGN}")
