...
QString Q_MG_BaseClass::Interpret( double val, bool isVertical,bool isVirtual, QString* out,QString In,double start,double end,bool startEndIsVirtual )
{
	...
	auto kiloMath = [&]()
	{
		if (((EnableKiloMath) && ( (isVertical) && (!EnableVertTimerConvertor) )) || ( (EnableKiloMath) && ( (!isVertical) && (!EnableHorTimerConvertor) )))
		{
			if (LineEtalon > 0)
			{
				if (LineEtalon >= 5000000000) {val = val/1000000000; addedLabel = QString::fromLocal8Bit("Г"); return;}
				if (LineEtalon >= 5000000) {val = val/1000000; addedLabel = QString::fromLocal8Bit("М");return;}
				if (LineEtalon >= 5000) {val = val/1000; addedLabel = QString::fromLocal8Bit("к");return;}

				if (LineEtalon <= 0.0000009) {val = val*1000000000; addedLabel = QString::fromLocal8Bit("н"); return;}
				if (LineEtalon <= 0.0009) {val = val*1000000; addedLabel = QString::fromLocal8Bit("мк");return;}
				if (LineEtalon <= 0.9) {val = val*1000; addedLabel = QString::fromLocal8Bit("м");return;}
			}
			else
			{
				if (LineEtalon <= -5000000000) {val = val/1000000000; addedLabel = QString::fromLocal8Bit("Г");return;}
...
void Q_MG_SelectionAreas::SetSelection(int mode, double x1, double y1, double x2, double y2, bool noSignal)
{
	if(mode == 100) {
		m_redLine.insert(y1, x1);
		return;
	} else if(mode == 101) {
		m_redLine.clear();
		return;
	...
	typePoint pp;
	pp.point = QPointF(x1,x2);
	pp.type = mode;

	HAxisSelList.append(pp);
	ReDraw();
}
...
//	{
		QBrush oldBrush = painter->brush();
		QPen oldPen = painter->pen();
		QBrush SellFill = settings->StaticAreaFillColor;//цвет выделения
		QPen SellPen = settings->StaticAreaFillColor;//цвет бордюра выделения

		//статическое
		for (int z = 0;z < HAxisSelList.count();z++)//1
		{
			QPointF sel = HAxisSelList[z].point;

			if( HAxisSelList[z].type == 1 ) { //white
				SellFill = settings->StaticAreaFillColor;//цвет выделения
				SellPen = settings->StaticAreaFillColor;//цвет бордюра выделения
			} else if( HAxisSelList[z].type == 5 ){
				SellFill = settings->StaticAreaFillColorAnalysis;//цвет выделения
				SellPen = settings->StaticAreaFillColorAnalysis;//цвет бордюра выделения
			} else {
				SellFill = settings->StaticAreaFillColor2;//цвет выделения
				SellPen = settings->StaticAreaFillColor2;//цвет бордюра выделения
			}
...
//			}
		...
		foreach (int freq, m_redLine.keys()) {
			QPen redPen;
			redPen.setStyle(Qt::DotLine);
			redPen.setColor(Qt::red);
			painter->setPen(redPen);
			double yyy = GetPixelByValue(m_redLine.value(freq),true);
			int x1 = (freq*1000000) - 5*100000;
			int x2 = (freq*1000000) + 5*100000;
			x1 -= HorInterpretSum;
			x2 -= HorInterpretSum;
			double xxx1 = GetPixelByValue(x1, false);
			double xxx2 = GetPixelByValue(x2, false);
			painter->drawLine(xxx1, yyy, xxx2, yyy);
		}
