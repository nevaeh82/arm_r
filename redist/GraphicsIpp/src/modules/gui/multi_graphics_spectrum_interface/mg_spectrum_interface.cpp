...
void Q_MG_SpectrumInterface::InitMainGraf()
{
	...
	connect(material_1, SIGNAL(plotReady()), this, SIGNAL(plotReady()));
...
void Q_MG_SpectrumInterface::SetDetectedAreas(int mode, double x1, double y1, double x2, double y2, bool noSignal)
{
	MainGraf->DetectedLayer->SetSelection(mode, x1,y1,x2,y2);
}
...
void Q_MG_SpectrumInterface::SetLabel( double valX_b,QString text,QString name,bool immediately_update/* = false*/, QColor label_color )
{
	...
	if(text == "resetRedLine") {
		if (MainGraf->DetectedLayer == nullptr) return;
		MainGraf->DetectedLayer->SetSelection(101, 0, 0, 0, 0);
		return;
	}

	if(text == "redLine") {
		if (MainGraf->DetectedLayer == nullptr) return;
		MainGraf->DetectedLayer->SetSelection(100, valX_b, name.toInt(), 0, 0);
		return;
	}
