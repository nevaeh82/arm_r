...
# Defines
!define REGKEY "SOFTWARE\<xsl:value-of  select="product/@code"/>-<xsl:value-of  select="product/@version"/>"
...
${If} $InstDir == "" ; /D= was not used on the command line
    StrCpy $InstDir "<xsl:value-of select="installdir/@path"/>"
${EndIf}
FunctionEnd
...
; регистрация ключей деинсталлятора
