...
//	QString name = argument.toString();
	log_debug(QString("StName = %1").arg(name));
	return name;
}
...
{
	log_debug( QString("Message received from KTR, message %1").arg(method) );
	...
	if (false) {}
	...
	else if (method == RPC_METHOD_REQUEST_STATUS) {
		...
		log_info(QString("Connection state for %1 = %2").arg(getNameFromArgument(argument.toByteArray())).arg(connectionState));
	}
}
