	...
	QWidget(parent, flags), ui(new Ui::GraphicsWidget)
{
	...
	ui->spectrumWidget->SetHorizontalLabel("Ãö");
...
void GraphicWidget::_slotSetFFTSetup(float* spectrum, float* spectrum_peak_hold)
{
	...
	ui->spectrumWidget->Setup(_isComplex,_bandwidth,"Óðîâåíü", spectrum, _pointCount, spectrum_peak_hold, _pointCount,false, false, minv, maxv);
	_mux.unlock();
...
void GraphicWidget::slotSetCorrelations(int id, const QVector<QPointF> vecFFT, const bool isComplex)
{
	...
	else
	{
		...
		//            _spectrum_peak_hold_corr[i] = spectrum[i];
		//        }
		//        spectrumWidget->Setup(isComplex,bandwidth,"Óðîâåíü",spectrum,PointCount, 0/*_spectrum_peak_hold_corr*/,0/*PointCount*/,false, false, minv, maxv);
	}
	delete[] spectrum;
}
