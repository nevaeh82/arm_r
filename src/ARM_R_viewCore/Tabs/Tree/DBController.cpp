...
int DBController::_set_db()
{
	...
	if(!readSettings(db_setting_file)){
		return -1;
	}
...
bool DBController::readSettings(const QString& settingsFile)
{
	QSettings m_settings(settingsFile, QSettings::IniFormat);

	_db_name.append("DB/");
	_db_name += m_settings.value("DB_file_name/file", "db_tree.sqlite").toString();
