	...
	RpcRoutedServer( parent )
{
	...
	m_serverPeer->attachSlot(RPC_METHOD_SEND_CPPACKET_PROTO, this, SLOT(slotSetCPPacketProtoSetup(quint64, QByteArray)));
	m_serverPeer->attachSlot(RPC_METHOD_SEND_SERVER_REQUEST_SETTINGS, this, SLOT(slotServerRequestSettings(quint64, int)));
...
void RpcServer::slotSetCPPacketProtoSetup(quint64 client, QByteArray data)
{
	dispatch( RPC_METHOD_SEND_CPPACKET_PROTO, QVariant(data), 0 );
}

void RpcServer::slotServerRequestSettings(quint64 client, int id)
{
	dispatch( RPC_METHOD_SEND_SERVER_REQUEST_SETTINGS, QVariant(id), 0 );
}
